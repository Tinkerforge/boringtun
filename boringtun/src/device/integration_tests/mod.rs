...
mod tests {
    ...
    impl Peer {
        ...
        fn new(endpoint: SocketAddr, allowed_ips: Vec<AllowedIp>) -> Peer {
            Peer {
                key: StaticSecret::random_from_rng(aead::OsRng),
                endpoint,
                allowed_ips,
                container_name: None,
            }
        ...
        fn gen_wg_conf(
            ...
            let _ = writeln!(conf, "PrivateKey = {}", STANDARD.encode(self.key.to_bytes()));

            // We are the peer
            let _ = writeln!(conf, "[Peer]");
            let _ = writeln!(conf, "PublicKey = {}", STANDARD.encode(local_key.as_bytes()));
    ...
    fn test_wireguard_set() {
        let port = next_port();
        let private_key = StaticSecret::random_from_rng(aead::OsRng);
        ...
        let peer_key = StaticSecret::random_from_rng(aead::OsRng);
